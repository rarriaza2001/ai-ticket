...
class DraftSuggestionRepository:
    ...
    async def get_latest_for_ticket(self, ticket_id: uuid.UUID) -> DraftSuggestion | None:
        result = await self._session.execute(
            select(DraftSuggestion)
            .where(DraftSuggestion.ticket_id == ticket_id)
            .order_by(DraftSuggestion.created_at.desc())
            .limit(1)
        )
